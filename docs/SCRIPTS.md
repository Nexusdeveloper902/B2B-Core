...
with most `extension=` lines **commented out**, so a fresh install fails
Laravel's extension requirements until they are enabled. `./run doctor` detects
this and prints the exact fix:
