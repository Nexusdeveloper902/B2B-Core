# Realtime feed — WebSocket contract (TASK-016, ADR-026)

The dashboards update live: when a student taps a card, every open
dashboard sees the tap within ~300 ms — no F5. This document is the
protocol contract.
...
- `{"type":"hello","last_id":N,"events":[…]}` — sent once after the
  101; `events` = the recent history (oldest first, `history_limit`
  rows), the same rows the page server-renders.
...
- Badge honesty: `live` / `connecting` / `offline` states; offline
  shows "reload to see the latest taps" instead of pretending.
...
- Page hook: `document` CustomEvent **`realtime:tap`** (detail = the
  ...
  server's earliest-event semantics).

## Degradation honesty

| Failure | Behavior |
|---|---|
| realtime:serve down / port busy | page still renders (SSR rows); badge shows Offline + hint |
...
- `tests/Unit/Realtime/*` — RFC 6455 spec vector, frame codec
  ...
  verify/tamper/expiry, feed row shaping.
...
- `tests/Feature/Realtime/RealtimeServerTest` — the REAL server
  ...
  a row written by another process, 401 on bad token.
